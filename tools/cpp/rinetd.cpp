...
int main(int argc,char *argv[])
{
    if (argc != 4)
    {
        ...
        printf("Sample:./rinetd /log/rinetd/rinetd.log /etc/rinetd.conf 5001\n\n");
        printf("        /home/lz6years/backend/myproject/tools/bin/procctl 5 /home/lz6years/backend/myproject/tools/bin/rinetd /log/rinetd/rinetd.log /etc/rinetd.conf 5001\n\n");
    ...
    // 建立连接后把监听socket设置成非阻塞。
    // fcntl(cmdlistensock, F_SETFL, fcntl(cmdlistensock, F_GETFD, 0) | O_NONBLOCK);
    ...
    while(true)
    {
        ...
        for (int i = 0; i < infds; ++i)
        {
            ...
            for (j = 0; j < vroute.size(); ++j)
            {
                ...
                {
                    // 1) 从已连接队列中获取一个已准备好的外网客户端的socket。
                    struct sockaddr_in client;
                    socklen_t len = sizeof(client);
                    ...
                    // 2) 先通过控制通道将需要连接的ip和端口传送到内网的服务器中
                    string sendbuffer = sformat("<dstip>%s</dstip><dstport>%d</dstport>", vroute[j].dstip, vroute[j].dstport);
                    if(send(cmdconnsock, sendbuffer.c_str(), sendbuffer.length(), 0) <= 0){
                        logfile.write("与内网的命令通道已断开。\n"); EXIT(-1);
                    }

                    // 3) 接受内网服务器的client请求连接
                    int dstsock = accept(cmdlistensock, (struct sockaddr *)&client, &len);
                    if (dstsock < 0) break;
                    if (dstsock >= MAXSOCK){
                        logfile.write("连接数已超过最大值%d。\n", MAXSOCK);
                        close(dstsock); close(srcsock);
                        break;
                    }
