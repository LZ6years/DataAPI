...
int main(int argc,char *argv[])
{
    ...
    ev.events=EPOLLIN;      // 打算让epoll监视listensock的读事件(水平触发)。
    // ev.events = EPOLLIN | EPOLLET;    // 打算让epoll监视listensock的读事件(边缘触发)。
    ...
    while (true)        // 事件循环。
    {
        ...
        for (int ii=0;ii<infds;ii++)       // 遍历epoll返回的数组evs。
        {
            ...
            else
            {
                ...
                // {
                //     if (send(evs[ii].data.fd, "dwakldanaaaaaaaaaaaaaaaaaaaaaaaa", 30, 0) <= 0)
                //     {
                //         if(errno == EAGAIN)
                //         {
                //             printf("发送缓冲区已经填满\n");
                ...
                // while (true)
                // {
                //     if ((readn = recv(evs[ii].data.fd, ptr, sizeof(buffer),0)) <= 0)
                //     {
                ...
                char buffer[1024]; // 存放从客户端读取的数据。
                memset(buffer,0,sizeof(buffer));
                if (recv(evs[ii].data.fd,buffer,1024,0)<=0)
                {
                    // 如果客户端的连接已断开。
                    printf("client(eventfd=%d) disconnected.\n",evs[ii].data.fd);
                    close(evs[ii].data.fd);            // 关闭客户端的socket
